...
const RowLeader = ({index,data}) =>
{
    return(
        <tr>
            ...
            <td>{data.totalMembers || 0}</td>
            <td>
                <NavLink to={`/employee/${data.id}`} className="btn btn-secondary btn-sm mr-2">Detail</NavLink>
                <button className="btn btn-danger btn-sm" onClick={() => (data.onDelete && data.onDelete(data))}>
